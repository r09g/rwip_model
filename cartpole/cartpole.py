...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    expectation = mdp_data['value'].dot(mdp_data['transition_probs'][state])
    
    # Return random action if no optimal
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for i in range(mdp_data['transition_counts'].shape[0]):
        ...
        # update estimated reward value in MDP, check to not divide 0
        if mdp_data['reward_counts'][i, 1] != 0:
            mdp_data['reward'][i] = mdp_data['reward_counts'][i, 0] * (-1) / mdp_data['reward_counts'][i, 1]
        
        # update estimated transition prababilities in MDP
        for j in range(mdp_data['transition_counts'].shape[2]):
            num_state_action = np.sum(mdp_data['transition_counts'][i, :, j])
            
            # check to not divide 0
            if num_state_action != 0:
                mdp_data['transition_probs'][i, :, j] = mdp_data['transition_counts'][i, :, j] / num_state_action   
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        new_value = mdp_data['reward'] + gamma * (np.max(old_value.dot(mdp_data['transition_probs']), axis=1))
        mdp_data['value'] = new_value
        count = count + 1
        
        # check convergence
        abs = np.abs(new_value - old_value)
        if np.max(abs) < tolerance:
            break
